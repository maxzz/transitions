...
export function ResponseGraph() {
    ...
    const definition = engineDefinitions[result.engineId];
    const overshoot = Math.max(0, graph.bounds.maxValue - 1);
    const durationLabel = result.engineId === "gsap" ? "fixed duration" : "settled after";

    return (
        <div className="h-full min-h-[28rem] bg-muted/20 flex flex-col">
