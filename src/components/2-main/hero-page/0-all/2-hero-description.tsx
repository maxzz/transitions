...
export function HeroDescription({ typed }: { typed: MotionValue<string>; }) {
    ...
    return (
        <p className="mt-6 max-w-xl min-h-24 text-sm sm:text-base text-muted-foreground text-center leading-relaxed contain-[layout]" aria-label={HERO_DESCRIPTION}>
            <motion.span aria-hidden>{typed}</motion.span>

            {showCaret && (
                <motion.span
                    className="ml-0.5 w-[0.55ch] inline-block"
                    animate={{ opacity: [1, 0] }}
